...
// RootCmd est la commande racine Cobra, exportée pour que les sous-commandes
// puissent s'y rattacher depuis leurs propres init().
...
func Execute() {
	if err := RootCmd.Execute(); err != nil {
		log.Fatalf("FATAL: échec de l'exécution de la commande: %v", err)
	}
}
...
func init() {
	// Initialise la configuration au démarrage de Cobra.
	cobra.OnInitialize(initConfig)

	// Déclare la commande racine pour que les sous-commandes
	// puissent s'y rattacher via cmd.RootCmd.AddCommand() dans leurs init().
